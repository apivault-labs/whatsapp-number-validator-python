"""Thin synchronous client for the WhatsApp Number Validator Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, WhatsAppNumberValidatorError

ACTOR_ID = "apivault_labs~whatsapp-number-validator"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class WhatsAppNumberValidatorClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "whatsapp-number-validator-python/0.1.0",
        })
    ...
    @staticmethod
    def estimate_cost(result_count: int, price_per_1000: float = 3.0) -> float:
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise WhatsAppNumberValidatorError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise WhatsAppNumberValidatorError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise WhatsAppNumberValidatorError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
